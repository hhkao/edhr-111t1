...
drev_P_load <- merge(x = drev_person_1, y = data_load, by = c("organization_id", "idnumber"), all.x = TRUE, all.y = TRUE) %>%
  ...
  mutate(P_load = if_else(!is.na(tutor), 1, 0)) #P_load = 1：有填教學資料表
...
flag_load$err_flag <- if_else(flag_load$source == "教員資料表" & (flag_load$tutor == "Y" | flag_load$tutor == "N") & (flag_load$sertype == "教師" | flag_load$sertype == "主任教官" | flag_load$sertype == "教官") & (is.na(flag_load$depcode_p) & flag_load$P_load == 1), 1, flag_load$err_flag)
flag_load$err_flag <- if_else(flag_load$source == "教員資料表" & (flag_load$tutor == "Y" | flag_load$tutor == "N") & (flag_load$sertype == "教師" | flag_load$sertype == "主任教官" | flag_load$sertype == "教官") & (flag_load$depcode_p == "NA" & !is.na(flag_load$depcode_p)), 0, flag_load$err_flag)  #因NA要改成flag31處理<這裡指主要任教科別直接填寫"NA">
...
if (dim(flag_load %>% subset(err_flag == 1))[1] != 0){
  ...
  flag25_temp <- flag_load_wide_flag25 %>%
    group_by(organization_id) %>%
    mutate(flag25_txt = paste("請確認該員之科別名稱(非授課科目)或所歸屬之領域/群別（可參閱欄位說明及其附件）：", flag25_r, sep = ""), "") %>%
